    ...
    <select class="form-control @if(isset($class)){{ $class }} @endif" name="{{ $name }}" @if(false !== strpos($name, '[]')) multiple @endif @if(isset($enabled) && (!$enabled)) disabled @endcannot @if(isset($id)) id="{{ $id }}_input" @endif @if(isset($placeholder)) placeholder="{{ $placeholder }}" @endif>
        ...
        @foreach ($items as $key => $item)
                ...
                @else
                    ...
                    <option @if(is_object($item) && isset($item->id))value="{{ $item->id }}" @else value="{{ $checkValue }}" @endif @if(isset($value) && ($value == $checkValue)) selected @endif>@if(isset($item->name)){{ $item->name }} @elseif(isset($item->title)){{ $item->title }}@else{{$item}}@endif</option>
                @endif
        @endforeach
    </select>
