    ...
    <form method="post" action="{{ route('absences.update', $absence->id) }}">
        ...
        @component('components.ui.card')
            @slot('cardFooter')
                ...
                <form class="form-inline" style="display: inline;" action="{{ route('absences.destroy', $absence->id)}}"
                      method="post">
                    @csrf
                    @method('DELETE')
            ...
            <div id="frmReplacements">
                ...
                @foreach($absence->replacements as $replacement)
                    <div class="row replacement-row" id="replacement-row-{{ $loop->index }}"
                        ...
                        <div class="col-3">
                            <input type="text" class="form-control input-to" name="replacement[{{ $loop->index }}][to]"
                                   value="{{ $replacement->to->format('d.m.Y') }}" placeholder="TT.MM.JJJJ"/>
                        </div>
                        <div class="col-2 pull-right">
                            <button class="btn btn-sm btn-danger btnDeleteReplacement" title="Vertretung entfernen">
