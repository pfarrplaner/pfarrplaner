            ...
            <div class="card-body">
                ...
                <form method="post" action="{{ route('absences.update', $absence->id) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="reason" value="{{ $absence->reason }}"  placeholder="z.B. Urlaub, Fortbildung, o.ä."/>
                    </div>
                    <h3>Vertretungen</h3>
                    <div id="frmReplacements">
                        ...
                        @foreach($absence->replacements as $replacement)
                            <div class="row replacement-row" id="replacement-row-{{ $loop->index }}" data-row="{{ $loop->index }}">
                                <div class="col-4">
                                    <div class="form-group">
                                        <select class="form-control peopleSelect replacement-user" name="replacement[{{ $loop->index }}][user][]" multiple>
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}" @if($replacement->users->contains($user)) selected @endif>{{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-3" id="replacement-row-{{ $loop->index }}-from">
                                    <input type="text" class="form-control input-from" name="replacement[{{ $loop->index }}][from]" value="{{ $replacement->from->format('d.m.Y') }}" placeholder="TT.MM.JJJJ" />
                                </div>
                                <div class="col-3">
                                    <input type="text" class="form-control input-to" name="replacement[{{ $loop->index }}][to]" value="{{ $replacement->to->format('d.m.Y') }}" placeholder="TT.MM.JJJJ" />
                                </div>
                                <div class="col-2 pull-right">
                                    <button class="btn btn-sm btn-danger btnDeleteReplacement" title="Vertretung entfernen"><span class="fa fa-trash"></span></button>
                                </div>
                            </div>
                        @endforeach
                    </div>
