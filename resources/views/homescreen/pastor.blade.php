...
@section('title')Willkommen, @if($user->first_name != ''){{$user->first_name}}! @else {{ $user->name }}! @endif
@endsection

@section('content')
    <a class="btn btn-primary btn-lg" href="{{ route('calendar') }}"><span class="fa fa-calendar"></span> Zum
        Kalender</a>
    <a class="btn btn-secondary btn-lg" href="{{ route('baptisms.create') }}"><span class="fa fa-water"></span> Taufe
        anlegen...</a>
    <a class="btn btn-secondary btn-lg" href="{{ route('funerals.wizard') }}"><span class="fa fa-cross"></span>
        Beerdigung anlegen...</a>
    <a class="btn btn-secondary btn-lg" href="{{ route('weddings.wizard') }}"><span class="fa fa-ring"></span> Trauung
        anlegen...</a>
    <hr/>

    <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item active">
            <a class="nav-link active" href="#services" role="tab" data-toggle="tab">Meine
                Gottesdienste @if($services->count())<span
                        class="badge badge-primary">{{ $services->count() }}</span> @endif </a>
        </li>
        @canany(['gd-kasualien-lesen', 'gd-kasualien-bearbeiten'])
            <li class="nav-item">
                <a class="nav-link" href="#baptisms" role="tab" data-toggle="tab">Meine Taufen @if($baptisms->count())
                        <span class="badge badge-primary">{{ $baptisms->count() }}</span> @endif </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#baptismRequests" role="tab"
                   data-toggle="tab">Taufanfragen @if($baptismRequests->count())<span
                            class="badge badge-primary">{{ $baptismRequests->count() }}</span> @endif </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#funerals" role="tab" data-toggle="tab">Meine
                    Beerdigungen @if($funerals->count())<span
                            class="badge badge-primary">{{ $funerals->count() }}</span> @endif </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#weddings" role="tab" data-toggle="tab">Meine
                    Trauungen @if($weddings->count())<span
                            class="badge badge-primary">{{ $weddings->count() }}</span> @endif </a>
            </li>
        @endcanany
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach ($baptismRequests as $baptism)
                            <tr class="">
                                @include('partials.baptism.details', ['baptism' => $baptism])
                                <td>
                                    <a class="btn btn-sm btn-primary" href="{{route('baptisms.edit', $baptism->id)}}"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-secondary"
                                           ...
                                           title="Formular für Kirchenregisteramt"><span class="fa fa-file-pdf"></span></a>
                                    </td>
                                    @if ($loop->first)
                                        <td rowspan="{{ $service->funerals->count() }}">
                                            <a class="btn btn-sm btn-secondary"
                                            ...
                                            @can('gd-kasualien-bearbeiten')
                                                <a class="btn btn-sm btn-primary"
                                                   ...
                                                   title="Beerdigung bearbeiten"><span class="fa fa-edit"></span> <span
