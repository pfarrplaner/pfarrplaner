...
@else
    ...
    title="max. {{ $seating['grid'][$place]->seats.($seating['grid'][$place]->seats > 1 ? ' Personen':' Person').(isset($taken) ? ($auto ? ' (vorläufig zugewiesen)' : ' (manuell zugewiesen)') : '') }}">
        {{ $place }} @if(is_numeric($place) && ($seating['grid'][$place]->seats > 1))<span class="fa fa-couch"></span> @else @for($i=1;$i<=$seating['grid'][$place]->seats;$i++)<span class="fa fa-chair"></span>@endfor @endif
        </span>
@endif
